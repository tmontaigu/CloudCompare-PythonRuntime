...
/// Loading plythons plugins may be slow,
/// we want cloudcompare the open as fast as possible
/// so we laod plugins in a separate thread
...
void PythonPlugin::setMainAppInterface(ccMainAppInterface *app)
{
    ...
    // We load the plugins in a separate thread to avoid blocking the UI
    // However, the thread that will load plugins will need to acquire the GIL,
    // which the interpreter on the main thread current owns.
